...
```{r surv-cleaning}
surv_long <- surv_se %>% 
  select(!c(1:6, "audit_sum", "country", "week", "n", "weight", "popweight")) %>% 
...
```{r}
surv_long$agegr[surv_long$agegr == "1"] <- "18-34 years"
surv_long$agegr[surv_long$agegr == "2"] <- "35-54 years"
surv_long$agegr[surv_long$agegr == "3"] <- "55-98 years"

surv_long %>% 
  filter(country_label == "Sweden",
         covid_survey == "distress") %>% 
  ...
  ggplot(aes(x = score,
             ...
             group = score)) +
  geom_col() +
  facet_wrap(vars(agegr), ncol = 3, scale = "free_y") +
  scale_fill_manual(values = c("#bebada", "#bebada")) +
  scale_x_discrete(labels = c("Yes", "No")) +
  geom_text(aes(label = count, fill = NULL, vjust = 1.5),
            size = 3,
            colour = "white") +
  theme_light() +
...
#### Financial Status
```{r}
surv_long$residence[surv_long$residence == "1"] <- "Village/fram"
surv_long$residence[surv_long$residence == "2"] <- "Small city"
surv_long$residence[surv_long$residence == "3"] <- "Medium-size city"
surv_long$residence[surv_long$residence == "4"] <- "Large city"
surv_long$residence[surv_long$residence == "5"] <- "Very large city"


surv_long %>% 
  filter(country_label == "Sweden",
         covid_survey == "change_financial",
         residence != "." ) %>% 
